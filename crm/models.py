...
class Customer(models.Model):
    ...
    department = models.CharField('院系/科室（全称）', max_length=20, default='')
    address = models.CharField('办公地址', max_length=50)
    title = models.CharField('职务', max_length=50, default='')
    # 提示需要添加默认值
    contact = models.PositiveIntegerField('联系方式', default=0)
    email = models.EmailField('邮箱')
    level = models.IntegerField('客户分级', choices=LEVEL_CHOICES, default=1)
...
class Intention(models.Model):
    ...
    project_type = models.IntegerField(
        '项目类型', choices=TYPE_CHOICES, default=1
    )
    amount = models.IntegerField('数量')
    # closing_date = models.DateField('预计成交时间', default=date.today())
    closing_date = models.DateField('预计成交时间', auto_now_add=True)
...
class IntentionRecord(models.Model):
    ...
    record_date = models.DateField('跟进时间', auto_now_add=True)
